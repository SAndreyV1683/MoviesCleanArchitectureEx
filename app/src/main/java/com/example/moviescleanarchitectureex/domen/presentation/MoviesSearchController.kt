package com.example.moviescleanarchitectureex.domen.presentation
            ...
            moviesInteractor.searchMovies(queryInput.text.toString(), object : MoviesInteractor.MoviesConsumer {
                override fun consume(foundMovies: List<Movie>?, message: String?) {
                    handler.post {
                        ...
                        if (message != null) {
                            showMessage(getString(activity, R.string.something_went_wrong), "")
                        }

                        if (movies.isEmpty()) {
                            showMessage(getString(activity, R.string.nothing_found), "")
                        } else {
                            hideMessage()
                        }
                    }
